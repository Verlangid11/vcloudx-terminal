...
RUN cd /tmp && wget -q https://go.dev/dl/go${GO_VERSION}.linux-amd64.tar.gz \
...
RUN cd /tmp && wget -q https://github.com/oven-sh/bun/releases/latest/download/bun-linux-x64.zip \
...
RUN mkdir -p $PLAYWRIGHT_BROWSERS_PATH \
    ...
    && npm install -g playwright pm2 nodemon pnpm yarn npm@latest typescript ts-node \
